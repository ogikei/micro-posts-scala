...
class UserFollowServiceImpl extends UserFollowService {
  ...
  override def findById(userId: Long)(implicit dbSession: DBSession = AutoSession)
  : Try[List[UserFollow]] = Try {
    UserFollow.where('userId -> userId).apply()
  }

  override def findByFollowId(followId: Long)(implicit dbSession: DBSession = AutoSession)
  : Try[Option[UserFollow]] =
    Try {
  ...
  override def countByFollowId(userId: Long)(implicit dbSession: DBSession = AutoSession)
  : Try[Long] = Try {
  ...
  override def countByUserId(userId: Long)(implicit dbSession: DBSession = AutoSession)
  : Try[Long] = Try {
    UserFollow.allAssociations.countBy(sqls.eq(UserFollow.defaultAlias.userId, userId))
  }

  override def deleteBy(userId: Long, followId: Long)(implicit dbSession: DBSession = AutoSession)
  : Try[Int] = Try {
